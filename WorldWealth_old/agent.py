...
class Person:
    def __init__(self) -> None:
        ...
        self.genes = {
            ...
            'premature death':  np.random.choice(a=[True, False], p=PROBABILITY_DIE_ACCIDENT),
            'invest': np.random.choice(a=[True, False], p=PROBABILITY_INVEST)
        }
        self.invest_percentage = np.random.uniform(0.05, 0.2, size=1)[0]
        self.years_to_give_birth = helpers.birth_age()
        self.year_premature_die = int(
            np.random.normal(loc=30, scale=25, size=1))
    ...
    def feed_children(self):
        for child in self.children:
            ...
            if child.genes['study'] and child.age < WORKING_AGE:
                if self.have_money(FEED_CHILD):
                    child.money += FEED_CHILD
                    self.money -= FEED_CHILD
    ...
    def dead(self):
        ...
        if self.genes['premature death']:
            if self.age == self.year_premature_die:
                self.alive = False
